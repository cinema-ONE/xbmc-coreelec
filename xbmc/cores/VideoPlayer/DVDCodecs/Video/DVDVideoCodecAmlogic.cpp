...
bool CDVDVideoCodecAmlogic::AddData(const DemuxPacket &packet)
{
  ...
  if (pData)
  {
    ...
    // MVC is SDR, so this sits outside the HDR gate above. Gate on the stream
    // being stereoscopic: DemuxPacket::subtitlePlane defaults to 0, so a
    // >= 0 test would scan every H.264 stream for an OFMD that cannot be there.
    if (m_hints.codec == AV_CODEC_ID_H264 && !m_hints.stereo_mode.empty())
    {
